...
class UnitImporterService
{
    ...
    public function processUnits(array $units): void
    {
        foreach ($units as $rowData) {
            ...
            if ($this->isOverwritable()) {
                if (null !== $unit) {
                    $this->logInfo('Updating unit with id: ' . $rowData[0] . ' and name: ' . $rowData[2] . ' and type: ' . $rowData[3]);
    ...
    {
        $this->logInfo('Creating unit with id: ' . $rowData[0] . ' and name: ' . $rowData[2] . ' and type: ' . $rowData[3]);
        return new Unit(
